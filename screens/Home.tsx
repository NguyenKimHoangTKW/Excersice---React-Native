...
const Home = ({ navigation, route }) => {
    const [myBookstest, setMyBookstest] = React.useState([]);
    const [categories, setCategories] = React.useState([]);
    const [username, setUsername] = React.useState("");
    async function books() {
        const response = await fetch(`${BASE_URL}/danh-sach-books`,
            {
                method: "GET",
                headers: {
        ...
        if (data.success) {
            setMyBookstest(data.data);
        }
    }
    async function LoadCategories() {
        const response = await fetch(`${BASE_URL}/danh-sach-categories`,
            {
                method: "GET",
                headers: {
                    "Content-Type": "application/json"
            ...
            <View style={{ flex: 1, flexDirection: 'row', paddingHorizontal: SIZES.padding, alignItems: 'center' }}>
                ...
                >
                    <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
                        ...
                        <Text style={{ marginLeft: SIZES.base, color: COLORS.white, ...FONTS.body3 }}>200 point</Text>
                    </View>
                </TouchableOpacity>
            </View>
        )
    ...
    function renderMyBookSection() {
        // Map để xử lý require ảnh tĩnh
        const bookCoverMap = {
            "other_words_for_home.jpg": require('../assets/images/other_words_for_home.jpg'),
            "the_metropolist.jpg": require('../assets/images/the_metropolist.jpg'),
            "the_tiny_dragon.jpg": require('../assets/images/the_tiny_dragon.jpg'),
        }
        ...
        const renderItem = ({ item, index }) => {
            return (
                <TouchableOpacity
                    ...
                    {/* Book Cover */}
                    <Image
                        source={bookCoverMap[item.bookCover]}
                        resizeMode="cover"
                        style={{
                            width: 180,
                            height: 250,
